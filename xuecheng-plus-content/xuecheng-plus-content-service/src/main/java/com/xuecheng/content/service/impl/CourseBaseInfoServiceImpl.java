...
@Transactional
    @Override
    public CourseBaseInfoDto updateCouseBaseInfo(Long companyId, EditCourseDto editCourseDto) {
        CourseBase courseBase = courseBaseMapper.selectById(editCourseDto.getId());
        if (courseBase == null) {
            XueChengPlusException.cast("没有该课程");
        }
        if (!courseBase.getCompanyId().equals(companyId)) {
            XueChengPlusException.cast("不能修改非本机构课程");
        }
        ...
        if (i!=1)
        {
            XueChengPlusException.cast("基础信息更新失败");

        }

        CourseMarket courseMarket = courseMarketMapper.selectById(editCourseDto.getId());
        if (courseMarket == null) {courseMarket=new CourseMarket();}
...
@Transactional
    @Override
    public void deleteCourse(Long courseId, Long compneyId) {
        CourseBase courseBase = courseBaseMapper.selectById(courseId);
        if (courseBase == null) {
        ...
        if (!courseBase.getCompanyId().equals(compneyId)) {
            XueChengPlusException.cast("非本机构老师不能删除课程");

        }
        String status = courseBase.getAuditStatus();
        if (!status.equals("202002")) {
            XueChengPlusException.cast("课程审核已提交，不能删除");
        }
    ...
    courseTeacherMapper.deleteById(courseId);
